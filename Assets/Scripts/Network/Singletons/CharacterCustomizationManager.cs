...
namespace Network.Singletons {
    ...
    public class CharacterCustomizationManager : MonoBehaviour {
        ...
        private void OnEnable() {
            if(_uiDocument == null) {
                _uiDocument = mainMenuManager?.uiDocument ?? FindFirstObjectByType<UIDocument>();
            }
        ...
        private void SetupUIReferences() {
            ...
            _colorPreviewBox = _colorPreviewButton ?? _root.Q<VisualElement>("color-preview-box");
        ...
        private void SetupEventHandlers() {
            // Color sliders
            _colorRSlider?.RegisterValueChangedCallback(evt => OnColorRChanged(evt.newValue));
            _colorGSlider?.RegisterValueChangedCallback(evt => OnColorGChanged(evt.newValue));
            _colorBSlider?.RegisterValueChangedCallback(evt => OnColorBChanged(evt.newValue));

            // Color inputs
            _colorRInput?.RegisterValueChangedCallback(evt => OnColorRInputChanged(evt.newValue));
            _colorGInput?.RegisterValueChangedCallback(evt => OnColorGInputChanged(evt.newValue));
            _colorBInput?.RegisterValueChangedCallback(evt => OnColorBInputChanged(evt.newValue));

            // Color preview button -> open packet panel
            if(_colorPreviewButton != null) {
                _colorPreviewButton.RegisterCallback<MouseEnterEvent>(evt => MouseEnterCallback?.Invoke(evt));
            ...
            if(_materialPacketBackButton != null) {
                _materialPacketBackButton.RegisterCallback<MouseEnterEvent>(evt => MouseEnterCallback?.Invoke(evt));
            ...
            // Material sliders
            _smoothnessSlider?.RegisterValueChangedCallback(evt => OnSmoothnessChanged(evt.newValue));
            _metallicSlider?.RegisterValueChangedCallback(evt => OnMetallicChanged(evt.newValue));
            _heightSlider?.RegisterValueChangedCallback(evt => OnHeightStrengthChanged(evt.newValue));
            _emissionRSlider?.RegisterValueChangedCallback(evt => OnEmissionRChanged(evt.newValue));
            _emissionGSlider?.RegisterValueChangedCallback(evt => OnEmissionGChanged(evt.newValue));
            _emissionBSlider?.RegisterValueChangedCallback(evt => OnEmissionBChanged(evt.newValue));
            ...
            if(_emissionToggle != null) {
                _emissionToggle.RegisterCallback<MouseEnterEvent>(evt => MouseEnterCallback?.Invoke(evt));
                _emissionToggle.RegisterValueChangedCallback(evt => OnEmissionToggleChanged(evt.newValue));
            }

            if(_emissionPreviewButton != null) {
                _emissionPreviewButton.RegisterCallback<MouseEnterEvent>(evt => MouseEnterCallback?.Invoke(evt));
            }

            _emissionRInput?.RegisterValueChangedCallback(evt => OnEmissionRInputChanged(evt.newValue));
            _emissionGInput?.RegisterValueChangedCallback(evt => OnEmissionGInputChanged(evt.newValue));
            _emissionBInput?.RegisterValueChangedCallback(evt => OnEmissionBInputChanged(evt.newValue));
        ...
        private void UpdateColorUI() {
            ...
            _colorRSlider?.SetValueWithoutNotify(r);
            _colorGSlider?.SetValueWithoutNotify(g);
            _colorBSlider?.SetValueWithoutNotify(b);
            _colorRInput?.SetValueWithoutNotify(r);
            _colorGInput?.SetValueWithoutNotify(g);
            _colorBInput?.SetValueWithoutNotify(b);
        ...
        private void UpdateSmoothnessUI() {
            _smoothnessSlider?.SetValueWithoutNotify(_currentSmoothness);
            UpdateSmoothnessDisplay();
        }
        ...
        private void UpdateMetallicUI() {
            _metallicSlider?.SetValueWithoutNotify(_currentMetallic);
            UpdateMetallicDisplay();
        }
        ...
        private void UpdateHeightUI() {
            _heightSlider?.SetValueWithoutNotify(_currentHeightStrength);
            UpdateHeightDisplay();
        }
        ...
        private void UpdateEmissionUI() {
            _emissionToggle?.SetValueWithoutNotify(_currentEmissionEnabled);
        ...
        private void UpdateEmissionColorControls() {
            ...
            _emissionRSlider?.SetValueWithoutNotify(r);
            _emissionGSlider?.SetValueWithoutNotify(g);
            _emissionBSlider?.SetValueWithoutNotify(b);
            _emissionRInput?.SetValueWithoutNotify(r);
            _emissionGInput?.SetValueWithoutNotify(g);
            _emissionBInput?.SetValueWithoutNotify(b);
        ...
        private void SetEmissionControlsEnabled(bool propertyEnabled) {
            _emissionPreviewButton?.SetEnabled(propertyEnabled);
            _emissionRSlider?.SetEnabled(propertyEnabled);
            _emissionGSlider?.SetEnabled(propertyEnabled);
            _emissionBSlider?.SetEnabled(propertyEnabled);
            _emissionRInput?.SetEnabled(propertyEnabled);
            _emissionGInput?.SetEnabled(propertyEnabled);
            _emissionBInput?.SetEnabled(propertyEnabled);
        }
        ...
        private void BuildMaterialPacketGrid() {
            ...
            var packets = packetManager?.GetAllPackets();
            if(packets == null || packets.Count == 0) {
                var fallbackPacket = packetManager?.GetNonePacket() ?? ScriptableObject.CreateInstance<PlayerMaterialPacket>();
        ...
        private Button CreatePacketButton(string label, int index, bool isPlaceholder) {
            ...
            button.RegisterCallback<MouseEnterEvent>(evt => MouseEnterCallback?.Invoke(evt));

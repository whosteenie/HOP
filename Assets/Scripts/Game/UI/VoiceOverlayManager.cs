...
namespace Game.UI {
    public class VoiceOverlayManager : UIElementBase {
        ...
        private readonly Dictionary<ulong, Game.Player.PlayerController> _trackedPlayers = new();
        ...
        protected override void OnInitialize() {
            ...
            if (Steamworks.SteamClient.IsValid) {
                _localPlayerId = Steamworks.SteamClient.SteamId.ToString();
            }
        ...
        private void Update() {
            ...
            var allPlayers = FindObjectsByType<Game.Player.PlayerController>(FindObjectsSortMode.None);
            foreach (var player in allPlayers) {
                if (player != null && player.steamId.Value != 0 && !_trackedPlayers.ContainsKey(player.steamId.Value)) {
                    _trackedPlayers[player.steamId.Value] = player;
                    player.isPttActive.OnValueChanged += (prev, curr) => OnRemotePttChanged(player, curr);
                }
            }
        }
        ...
        /// </summary>
        private void OnRemotePttChanged(Game.Player.PlayerController player, bool isActive) {
            if (_overlayContainer == null || player == null) return;
            
            string idStr = player.steamId.Value.ToString();
        ...
        private void OnLocalPttStateChanged(bool isActive) {
            ...
            if (isActive) {
                if (!_activeSpeakerElements.ContainsKey(_localPlayerId)) {
                    var displayName = Steamworks.SteamClient.Name;
                    var steamId = Steamworks.SteamClient.SteamId.Value;
        ...
        private void OnSpeechDetected(VivoxParticipant participant) {
            ...
            string idStr = participant.PlayerId;
            ...
            bool isLocalPlayer = idStr == _localPlayerId;
            if (isLocalPlayer && SocialSettings.InputMode == VoiceInputMode.PushToTalk) {
                return;
            }
            
            // Skip muted players
            if (!isLocalPlayer && SocialSettings.IsMuted(idStr)) {
                return;
            }
            ...
            if (!isLocalPlayer) {
                ...
                var players = FindObjectsByType<Game.Player.PlayerController>(FindObjectsSortMode.None);
                foreach (var player in players) {
                    if (player != null && player.steamId.Value.ToString() == idStr) {
            ...
            if (isSpeaking) {
                if (!_activeSpeakerElements.ContainsKey(idStr)) {
                    var players = FindObjectsByType<Game.Player.PlayerController>(FindObjectsSortMode.None);
                    Game.Player.PlayerController foundPlayer = null;
                    foreach (var player in players) {
                        if (player != null && player.steamId.Value.ToString() == idStr) {
                            foundPlayer = player;
                            break;
        ...
        private void RemoveSpeakerEntry(string id) {
             if(_activeSpeakerElements.TryGetValue(id, out var entry)) {
                 _overlayContainer.Remove(entry);
                 _activeSpeakerElements.Remove(id);
             }
        }
        
        private async UniTaskVoid LoadAvatar(ulong steamId, VisualElement avatarElement) {

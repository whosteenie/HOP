...
namespace Game.Weapons {
    ...
    public class WeaponCameraController : MonoBehaviour {
        ...
        private void Awake() {
            // Get or create weapon camera
            if(weaponCamera == null) {
                var cameraObj = new GameObject("WeaponCamera");
                cameraObj.transform.SetParent(transform);
                weaponCamera = cameraObj.AddComponent<Camera>();
            }
        ...
        private void SetupWeaponCamera() {
            ...
            if(_mainSceneCamera == null) {
                Debug.LogError("[WeaponCameraController] Main scene camera not found! Make sure there's a camera tagged 'MainCamera' in the scene.");
                return;
            }
            ...
            if(mainCameraData != null) {
                ...
                Debug.Log($"[WeaponCameraController] Weapon camera added to main scene camera stack. Stack count: {mainCameraData.cameraStack.Count}");
            } else {
                Debug.LogError("[WeaponCameraController] Main scene camera doesn't have URP camera data!");
            }
            
            Debug.Log($"[WeaponCameraController] Weapon camera configured as Overlay. Weapon layer: {_weaponLayerMask}");
        }

...
class VolumetricCloudsEditor : VolumeComponentEditor
{
    ...
    private static ScriptableRendererData[] GetRendererDataList(UniversalRenderPipelineAsset asset = null)
    {
        try
        {
            ...
            // Fallback for newer URP versions where reflection can be brittle.
            if (asset.rendererDataList.Length > 0)
            {
                var rendererDataList = new ScriptableRendererData[asset.rendererDataList.Length];
                for (int i = 0; i < rendererDataList.Length; i++)
                    rendererDataList[i] = asset.rendererDataList[i];

...
public class VolumetricCloudsURP : ScriptableRendererFeature
{
    ...
    private bool isSkipReasonPrinted = false;
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        if (isVolumeActive && (renderingData.cameraData.cameraType == CameraType.Game || renderingData.cameraData.cameraType == CameraType.SceneView || isProbeCamera))
        {
            ...
            isLogPrinted = false;
            isSkipReasonPrinted = false;
        }
#if UNITY_EDITOR || DEBUG
        else if (!isVolumeActive && !isSkipReasonPrinted)
        {
            Debug.Log($"Volumetric Clouds URP: Skipping render pass for camera '{renderingData.cameraData.camera.name}' because the volume is not active. cameraType={renderingData.cameraData.cameraType}, debugger={isDebugger}, renderingDebugger={renderingDebugger}, cloudsVolumePresent={(cloudsVolume != null)}, cloudsVolumeActive={(cloudsVolume != null && cloudsVolume.IsActive())}");
            isSkipReasonPrinted = true;
        }
        else if (!(renderingData.cameraData.cameraType == CameraType.Game || renderingData.cameraData.cameraType == CameraType.SceneView || isProbeCamera) && !isSkipReasonPrinted)
        {
            Debug.Log($"Volumetric Clouds URP: Skipping render pass for camera '{renderingData.cameraData.camera.name}' because cameraType={renderingData.cameraData.cameraType} is not supported and reflectionProbe={reflectionProbe}.");
            isSkipReasonPrinted = true;
        }
    ...
    public class VolumetricCloudsPass : ScriptableRenderPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<RasterPassData>(rasterPassProfilerTag, out var rasterPassData))
            {
                ...
                builder.UseTexture(rasterPassData.cameraColorHandle, AccessFlags.Read);
            ...
            using (var builder = renderGraph.AddUnsafePass<PassData>(profilerTag, out var passData))
            {
                ...
                RenderingUtils.ReAllocateHandleIfNeeded(ref accumulateHandle, desc, FilterMode.Point, TextureWrapMode.Clamp, name: _VolumetricCloudsAccumulationTexture);
                RenderingUtils.ReAllocateHandleIfNeeded(ref historyHandle, desc, FilterMode.Point, TextureWrapMode.Clamp, name: _VolumetricCloudsHistoryTexture);
                TextureHandle accumulateTextureHandle = renderGraph.ImportTexture(accumulateHandle);
                TextureHandle historyTextureHandle = renderGraph.ImportTexture(historyHandle);
                cloudsMaterial.SetTexture(volumetricCloudsHistoryTexture, historyHandle);
                ...
                if (outputToSceneDepth)
                {
                    ...
                    RenderingUtils.ReAllocateHandleIfNeeded(ref cameraTempDepthHandle, tempDepthDesc, FilterMode.Point, TextureWrapMode.Clamp, name: _CameraTempDepthTexture);
                    passData.cameraTempDepthHandle = renderGraph.ImportTexture(cameraTempDepthHandle);
                    builder.UseTexture(passData.cameraTempDepthHandle, AccessFlags.ReadWrite);
                }
                ...
                passData.denoiseClouds = denoiseClouds && cameraData.cameraTargetDescriptor.msaaSamples == 1;
                ...
                builder.UseTexture(passData.cameraDepthHandle, passData.outputToSceneDepth ? AccessFlags.ReadWrite : AccessFlags.Read);
    ...
    public class VolumetricCloudsShadowsPass : ScriptableRenderPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<PassData>(profilerTag, out var passData))
            {
                ...
                builder.UseTexture(passData.intermediateShadowTexture, AccessFlags.ReadWrite);

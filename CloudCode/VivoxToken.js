...
module.exports = async ({ params, context, logger, secretManager }) => {
  const issuerParam = (params && params.issuer) ? params.issuer : "";
  ...
  if (!issuer) {
    ...
    issuer = issuerSecret && issuerSecret.value ? issuerSecret.value : "";
  }

  const tokenKeySecret = await secretManager.getSecret("VIVOX_TOKEN_KEY");
  const secret = tokenKeySecret && tokenKeySecret.value ? tokenKeySecret.value : "";

  if (!issuer) {
    throw new Error("VivoxToken: missing issuer (set secret VIVOX_ISSUER or pass params.issuer).");
  }
